...
for num_shot in [1, 2, 4, 8, 16]:
    ...
    for seed in range(1, args.num_run + 1):
        ...
        for c_weight in search_list:
            clf = LogisticRegression(solver='lbfgs', max_iter=1000, penalty='l2', C=c_weight).fit(fewshot_train_feature,
                                                                                                  fewshot_train_label)
        ...
        def binary_search(c_left, c_right, seed, step, test_acc_step_list):
            clf_left = LogisticRegression(solver='lbfgs', max_iter=1000,
                                          penalty='l2', C=c_left).fit(fewshot_train_feature, fewshot_train_label)
            ...
            saveline = '{}, seed {}, {} shot, weight {}, test_acc {:.2f}\n'.format(dataset, seed,
                                                                                   num_shot, c_final, test_acc)
            with open('./report/{}_s{}r{}_details.txt'.format(args.feature_dir,
                                                                        args.num_step,
                                                                        args.num_run), 'a+') as writer:
                writer.write(saveline)
            return np.power(10, c_left), np.power(10, c_right), seed, step, test_acc_step_list

        for step in range(args.num_step):
            print(f'{dataset}, {num_shot} Shot, Round {step}: {c_left}/{c_right}', flush=True)
            c_left, c_right, seed, step, test_acc_step_list = binary_search(c_left, c_right,
                                                                            seed, step, test_acc_step_list)
    ...
    with open('./report/{}_s{}r{}.txt'.format(args.feature_dir, args.num_step, args.num_run), 'a+') as writer:
